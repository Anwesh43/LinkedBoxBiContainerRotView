...
class BoxBiCotainerRotView(ctx : Context) : View(ctx) {
    ...
    data class BBCRNode(var i : Int, val state : State = State()) {

        private var next : BBCRNode? = null
        private var prev : BBCRNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBBCRNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BBCRNode {
            var curr : BBCRNode ?= prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
